...
1. sync.WaitGroup 可以用来时协程同步，里面维护了一个计时器，add() 加，Done()减，只有计数器的值为0时，Wait后面的代码才会继续执行。
2. sync.Once实用只执行一次的环境中，例如配置加载。里面时互斥锁和一个布尔值
3. go 里面的map 在多goroutine 环境中时不安全的，可以使用sync.Map{}
