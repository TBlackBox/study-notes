...
//在调整大小的同时，要拆分下一个表索引（加一）
private transient volatile int transferIndex;

//当调整大小和/或创建CounterCells时，使用Spinlock（通过CAS锁定）。
...
final V putVal(K key, V value, boolean onlyIfAbsent) {
	...
	//ASHIFT ABASE 都是底层支持的  也就是上面字段里面的偏移值
...
（2）如果待插入的元素所在的桶为空，则尝试把此元素直接插入到桶的第一个位置（CAS更新）；

（3）如果正在扩容，则当前线程一起加入到扩容的过程中；

（4）如果待插入的元素所在的桶不为空且不在迁移元素，则锁住这个桶（分段锁）即`synchronized (f)`；
