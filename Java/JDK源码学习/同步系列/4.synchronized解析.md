...
编译后的sync()方法的字节码指令如下，为了便于阅读，特意加上了注释：
...
按照注释读起来，字节码比较简单，我们的synchronized锁定的是SynchronizedTest类对象，可以看到它从常量池中加载了两次SynchronizedTest类对象，分别存储在本地变量0和本地变量1中，解锁的时候正好是相反的顺序，先解锁变量1，再解锁变量0，实际上变量0和变量1指向的是同一个对象，所以synchronized是可重入的。
